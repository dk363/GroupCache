...
func TestGet(t *testing.T) {
	for _, tt := range getTests {
		...
		if ok != tt.expectedOK {
			t.Fatalf("%s: cache hit = %v; want %v", tt.name, ok, tt.expectedOK)
		} else if ok && val != 1234 {
...
func TestOnEvcted(t *testing.T) {
	evictedKeys := make([]Key, 0)
	var OnEvcted func(key Key, value interface{})
	OnEvcted = func(key Key, value interface{}) {
		evictedKeys = append(evictedKeys, key)
	}

	lru := LRUNew(20)
	lru.OnEvcted = OnEvcted
	for i := 0; i < 22; i++ {
		lru.Add(fmt.Sprintf("myKey%d", i), 1234)
	}

	if len(evictedKeys) != 2 {
		t.Fatalf("got %d eviected keys, expected 2", len(evictedKeys))
	}
